...
#get the part of image where are kidneys/tumors
#The left kidney is located slightly more superior than the right kidney - CT scan goes from bottom part of a body to a head
def get_relevant_interval(layers):
    ...
    #need array in (x,1) - not only in 1D
    
    #no occurence of specific color 
    #TODO more elegant way
    if( np.max(layers) == 0 and np.min(layers) == 0):
        #special case rmin -> larger then any of rmax (10000000,100000001,,...)
        #special case rmax -> smaller then any of rmin (-1,-2,-3...)
        return 10000000, -1
...
def create_bounding_box(new_volume, rmin_global, rmax_global, cmin_global, cmax_global):
    ...
    @param : global - biggest,smallest coordinates throught whole dataset
    """
