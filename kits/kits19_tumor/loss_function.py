...
def kidney_tumor_loss(y_true, y_pred):
    ...
    #return kidney_loss + tumor_loss + full_mask 
...
def softmax_dice_loss_tune_params(alpha, beta):
    def sdl(y_true, y_pred):
        #return alfa * categorical_crossentropy(y_true, y_pred) + beta * dice_coef_loss(y_true[..., 0], y_pred[..., 0]) + gama * dice_coef_loss(y_true[..., 2], y_pred[..., 2]) + lmbda * K.mean( K.abs( K.flatten(y_true) - K.flatten(y_pred) ) )
        return alpha * categorical_crossentropy(y_true, y_pred) + beta * dice_coef_loss(y_true, y_pred)        
    return sdl
...
def tversky_loss_adjusted(y_true, y_pred, alpha, beta, class_weights = None, smooth=0.000001):
...
def make_loss(loss_name,alpha,beta,class_weights = None):
    ...
    if loss_name == 'softmax_dice_loss':
        def loss(y, p):
        ...
        def loss(y, p):
            return tversky_loss_adjusted(y, p, alpha = alpha, beta = beta, class_weights = class_weights)
