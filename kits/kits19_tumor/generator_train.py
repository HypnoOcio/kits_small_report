...
#     'Generates data for Keras'
    def __init__(self, list_IDs,data_path,batch_size=4, dim=(512,512), rescale = 512, n_channels=3, shuffle=True):
    ...
    def get_batch(self):
        ...
        #rewrite internal index of generator
        
        if self.already_processed_files >= len(self.list_of_files):
            self.refresh_for_epoch()
            #raise ValueError('getitem in generator can not generate more files - adjust len_generator() -> number of batches through whole dataset')
            ##TODO calculate len of all data files into one file
        #else:
        #    num_of_file = list_of_files[already_processed_files]  
        ...
        #    vol_ims = cv2.resize(vol_ims, (128, 128))  # when scaling we scale original image to 128x128
        #    seg_ims = cv2.resize(seg_ims, (128, 128))  # when scaling we scale original image to 128x128
        if finished == True:
            #move because I finished one human
            self.already_processed_files += 1
            #do not need to have opened file anymore
    ...
    def get_photos_from_file(self,num_of_file):
        ...
        if num_of_file not in keys:
            vol, seg = load_data(self.data_path, num_of_file)
        ...
        file_finished = False
        #have I read the whole file yet?
...
def cut_off_background(x_train,y_train):
    """cut off images which contains only black color --- no kidney or tumor"""
